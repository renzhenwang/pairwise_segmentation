...
class ResNet(nn.Module):
    ...
    def __init__(self,  backbone, block, layers, output_stride, BatchNorm, in_channels=3, pretrained=True):
...
def resnet50(in_channels, output_stride, BatchNorm, pretrained=True):
    ...
    model = ResNet('resnet50', Bottleneck, [3, 4, 6, 3], output_stride, BatchNorm, 
...
def resnet101(in_channels, output_stride, BatchNorm, pretrained=True):
    if in_channels!=3 and pretrained:
        raise ValueError("pretraining is allowed only if input channels is 3")
    model = ResNet('resnet50', Bottleneck, [3, 4, 23, 3], output_stride, BatchNorm, 
                   in_channels=in_channels, pretrained=pretrained)
    return model
...
if __name__ == "__main__":
    import torch
    model = resnet50(in_channels=5, BatchNorm=nn.BatchNorm2d, pretrained=False,
                     output_stride=16)
